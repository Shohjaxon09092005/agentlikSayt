...
function Poll() {
  ...
  useEffect(() => {
    ...
    axios.get('https://agentlik-backend.onrender.com/api/v1/polls/')
      .then(response => {
        setPollData(response.data[2]); // Faqat birinchi so'rovnoma olinyapti
      })
      .catch(error => {
      ...
      {!voted ? (
        ...
        <div className="results">
          ...
          {pollData.choices.map(choice => (
            <div key={choice.id}>
              <p>{choice.answer}</p>
              <div className="progress-bar" style={{ width: `${calculatePercentage(choice.clicks)}%`, backgroundColor: 'grey' }}>
