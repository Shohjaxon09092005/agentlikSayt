...
function Elektron() {
  ...
  async function xalqData() {
    let fetchXalq = await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=elektron-hukumat`)
          ...
          <div className={`home_container ${isVisible ? "menu-visible" : ""}`}>
            ...
            <div className='all_home' style={{ lineHeight: '1.6' }}>
                ...
                {newsXalq?.map((item) => {
                  return (
                    <NewCard
                      ...
                      title={`${item?.title}`}
                      date={`${item?.created?.slice(0, item?.created.indexOf("T"))}`}
                      number={`${item?.views}`} newsNavTitle={`/news/${item?.id}`} NavIdNews={`/news/${item?.id}`}
                    />
                  );
                })}
              </div>
              ...
              <div className='web_icon' style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                ...
                <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objXalq?.created?.slice(0, objXalq?.created?.indexOf("T"))}</span>
              </div>
            </div>
          </div>
        </div>
