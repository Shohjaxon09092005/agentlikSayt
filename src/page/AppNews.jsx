...
async function jamData() {
    let fetchJam = await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=%09yangiliklar`);
        ...
        <div className='about'>
          ...
          <div className="side-menu">
            ...
            <div className="news-cards">
                {currentNews.map(n => (
                  <NewsCard key={n.id} news={n} newsNavTitle={`/news/${n?.id}`} NavIdNews={`/news/${n?.id}`} image={n?.images?.file} title={n.title} date={n?.created?.slice(0,n?.created?.indexOf("T"))} views={n.views}  />
                ))}
            </div>
