...
function AppForm() {
    ...
    async function pollData() {
      let fetchPoll=await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=murojaat-qoldirish`)
      let json=await fetchPoll.json()
      setPoll(json)
      
    }
    const objPoll=poll[0]
  return (
    <div>
                        ...
                        <HeaderService />
                                ...
                                <div className='web_icon' style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                        ...
                                        <span>{objPoll?.views}</span>
                                    </div>
                                    <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objPoll?.created?.slice(0,objPoll?.created?.indexOf("T"))}</span>
                                </div>
                            </div>
                        </div>
                    </div>
