...
function Document() {
    ...
    async function docData() {
      let fetchDoc=await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=%09ijtimoiy-ahamiyat`)
      let json =await fetchDoc.json()
      setDoc(json)
      
    }
    const simpleDoc=doc?.map(item=>item.simple_documents).flat()
    const objDoc=doc[0]
                    ...
                    <HeaderService />
                            ...
                            <div className='web_icon'  style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                    ...
                                    <span>{objDoc?.views}</span>
                                </div>
                                <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objDoc?.created.slice(0,objDoc?.created?.indexOf("T"))}</span>
                            </div>
                        </div>
                    </div>
                </div>
