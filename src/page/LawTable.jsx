...
function LawTable() {
    ...
    async function serData() {
        let fetchSer = await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=qonun-hujjatlari`);
        let json = await fetchSer.json()
        setSer(json)
    }
    const serDocum=ser?.map(item=>item.documents).flat()
    const sortedDocuments = [...serDocum].sort((a, b) => a.number - b.number);
    const objSer=ser[0]
    // console.log(ser);
    const filteredDocuments = sortedDocuments?.filter(doc =>
        doc?.category?.some(cat => cat.name === "O'zbekiston Respublikasining Qonunlari")
      );
      
      console.log(filteredDocuments);
    const filteredDocuments2 = sortedDocuments?.filter(doc =>
        doc?.category?.some(cat => cat.name === "O‘zbekiston Respublikasi Prezidentining Farmonlari")
      );
      
      console.log(filteredDocuments2);
                    ...
                    <HeaderService />
                            ...
                            <div className="jamoat_container">
                                <table className="law-table">
                                    ...
                                    <tbody>
                                        {filteredDocuments?.map((law) => (
                                            <tr key={law?.id}>
                                                <td>{law?.number}</td>
                                                <td>{law?.issued_date}</td>
                                                <td><a href={law?.link}>{law?.document_number}</a></td>
                                                <td>{law?.name}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                            ...
                            <div className='web_icon' style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                    ...
                                    <span>{objSer?.views}</span>
                                </div>
                                <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objSer?.created?.slice(0,10)}</span>
                            </div>
                        </div>
                    </div>
                </div>
