...
function Departaments() {
    ...
    async function depData(){
      let fetchDep=await fetch("https://agentlik-backend.onrender.com/api/v1/categories/page/?sub_category_slug=markaziy-apparat");
      let json=await fetchDep.json()
      setDep(json)
    }
    const staffsDep=dep?.map((item)=>item.staffs).flat()
    const obj = dep[0]
            ...
            <div ref={scrollHome} className={  `home_container ${isVisible ? "menu-visible" : ""}`}>
                ...
                <HeaderService />
                        ...
                        <div className="accordion-section">
                            {staffsDep && staffsDep?.map((item, index) => (
                        ...
                        <div className='web_icon' style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                            ...
                            <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {obj?.created?.slice(0,10)}</span>
                        </div>
                    </div>
                </div>
            </div>
