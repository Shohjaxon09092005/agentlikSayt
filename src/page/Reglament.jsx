...
function Reglament() {
  ...
  async function jamData() {
    let fetchJam = await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=%09ishonch-telefoni-reglamenti`);
        ...
        <div className="container">
          ...
          <div className={`home_container ${isVisible ? "menu-visible" : ""}`}>
            ...
            <div style={{ lineHeight: '1.6' }}>
              <h2 className='text_table_content'>Ishonch telefoni reglamenti</h2>
              <div className="jamoat_container">
                {serJam?.map((item) => {
                  return (
                    <p key={item?.id}>{item?.name}  <a style={{ color: "blue" }} href={item?.link} target="_blank" rel="noopener noreferrer">{item?.file_type}</a>.</p>
                  )
                })}
