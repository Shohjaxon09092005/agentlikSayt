...
function Agency() {
  ...
  async function imgNewsData() {
    let fetchImgNews=await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=%09agentlik-tuzilmasi`)
    let json=await fetchImgNews.json()
    setImgNews(json)
  }
  const pageImgNews=imgNews?.map(item=>item.pageimage_set).flat()
  const objImgNews=imgNews[0]
  console.log(objImgNews);
            ...
            <div className="container">
                <div className={`home_container ${isVisible ? "menu-visible" : ""}`}>
                    ...
                    <div className="divAgency">
                        ...
                        {pageImgNews?.map((item)=>{
                          return(
                             <img key={item?.id} className='agencyImg' src={item?.image} alt={item?.image} />
                          )
                        })}
                        ...
                        <div className='web_icon' style={{ display: 'flex', justifyContent:"space-around", alignItems: 'center', marginTop: '20px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                ...
                                <span>{objImgNews?.views}</span>
                            </div>
                            <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objImgNews?.created?.slice(0,objImgNews?.created?.indexOf("T"))}</span>
                        </div>
                    </div>
                    </div>
