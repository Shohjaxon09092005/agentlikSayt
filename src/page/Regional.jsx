...
function Regional() {
    ...
    const toggleAccordion = (index) => {
     ...
     async function regData(){
         let fetchReg=await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=hududiy-boshqarmalar`);
         let json= await fetchReg.json()
         setReg(json)
     }
     const staffsReg=reg?.map(item=>item.staffs).flat()
     const objReg=reg[0]
  return (
                ...
                <HeaderService />
                        ...
                        <div className="accordion-section">
                            {staffsReg?.map((item, index) => (
                                <div key={item?.id} className="accordion-item">
                                    ...
                                    {isOpen === index && (
                                        <ProfileCard proName={item?.full_name} proImg={item?.avatar} tel={item?.phone_number} mansab={item?.role?.description} webTitle={item?.website_url} location={item?.address} day={item?.reception_times} />
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className='web_icon' style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                ...
                                <span>{objReg?.views}</span>
                            </div>
                            <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objReg?.created?.slice(0,10)}</span>
                        </div>
                    </div>
                </div>
            </div>
