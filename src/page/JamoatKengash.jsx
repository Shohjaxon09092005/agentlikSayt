...
function JamoatKengash() {
    ...
    async function jamData() {
        let fetchJam = await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=jamoatchilik-kengashi`);
                ...
                <div ref={scrollHome} className={  `home_container ${isVisible ? "menu-visible" : ""}`}>
                    ...
                    <HeaderService />
                            ...
                            <div className='web_icon' style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: '20px' }}>
                                ...
                                <span style={{ fontSize: '14px', color: '#999' }}>Oxirgi yangilanish: {objJam?.created?.slice(0,10)}</span>
                            </div>
                        </div>
                    </div>
                </div>
