...
function Vakancy() {
  ...
  async function jamData() {
    let fetchJam = await fetch(`${URL}/api/v1/categories/page/?sub_category_slug=bosh-ish-orinlari`);
                    ...
                    <thead>
                      <tr>
                        {serJamTables_1?.map((item)=>{
                          return(
                            <th key={item?.id}>{item?.head?.title}</th>
                  ...
                  <ol className="additional-info">
                    {serJam?.map((item) => {
                      return (
                        <li>
                          {item?.name} /
                          <a href={item?.link}> {item?.file_type}</a>
                        </li>
                      )
                    })}
